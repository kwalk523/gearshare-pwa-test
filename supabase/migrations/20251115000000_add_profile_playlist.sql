-- Add all new profile feature columns to profiles table
-- This migration only adds new columns without modifying existing structure

DO $$ 
BEGIN
    -- Music & Playlist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='profile_playlist') THEN
        ALTER TABLE profiles ADD COLUMN profile_playlist text;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='profile_song') THEN
        ALTER TABLE profiles ADD COLUMN profile_song text;
    END IF;

    -- Basic Customization
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='profile_banner') THEN
        ALTER TABLE profiles ADD COLUMN profile_banner text;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='profile_pic') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='favorite_quote') THEN
        ALTER TABLE profiles ADD COLUMN favorite_quote text;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='font') THEN
        ALTER TABLE profiles ADD COLUMN font text DEFAULT 'font-sans';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='ring_color') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='mood_status') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='availability_message') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='grad_year') THEN
        ALTER TABLE profiles ADD COLUMN grad_year text;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='clubs') THEN
        ALTER TABLE profiles ADD COLUMN clubs text;
    END IF;

    -- Practical Features
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='wishlist') THEN
        ALTER TABLE profiles ADD COLUMN wishlist text;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='pickup_locations') THEN
        ALTER TABLE profiles ADD COLUMN pickup_locations text;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name='profiles' AND column_name='preferred_contact') THEN
