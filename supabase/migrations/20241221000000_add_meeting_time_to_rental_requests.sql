-- Add meeting_time column to rental_requests table and update the create_rental_request function

-- Add meeting_time column to rental_requests table
ALTER TABLE rental_requests ADD COLUMN IF NOT EXISTS meeting_time timestamptz;

-- Create index for meeting_time for better query performance
CREATE INDEX IF NOT EXISTS idx_rental_requests_meeting_time ON rental_requests(meeting_time);

-- Update the create_rental_request function to handle meeting_time
